...
import os
from ytproc import download_video, convert_to_audio
...
def test_cli_download_video():
    with patch('ytproc.download_video') as mock_download:
        ...
        with patch('sys.argv', ['ytproc.py', 'download', 'https://www.youtube.com/watch?v=test', '-o', 'output']):
            cli()
            mock_download.assert_called_once_with('https://www.youtube.com/watch?v=test', 'output.mp4')

def test_cli_download_audio():
    with patch('ytproc.download_video') as mock_download, \
         patch('ytproc.convert_to_audio') as mock_convert, \
         patch('os.remove') as mock_remove:
        from ytproc import cli
        with patch('sys.argv', ['ytproc.py', 'download', 'https://www.youtube.com/watch?v=test', '-o', 'output', '-a']):
            cli()
            mock_download.assert_called_once_with('https://www.youtube.com/watch?v=test', 'output.mp4')
